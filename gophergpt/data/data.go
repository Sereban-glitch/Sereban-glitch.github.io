...
// 🇷🇺 GopherGPT, Фаза 2: подмена вшитого корпуса внешним файлом.
// Merge-правила вшитого словаря обучены на английском тексте Верна и
// ссылаются на токены, которых в новом корпусе нет (паника в
// createMergeRules). Поэтому при подмене корпуса правила отключаем:
// новый корпус получает посимвольный словарь из своих уникальных символов.
func SetDataset(text string) {
        dataset = text
        vocab = ""
}

func Encode(s string) []float64 {
        return applyMerges(encodeChars(s, true))
}

// 🇷🇺 GopherGPT, фикс REPL: EncodeSafe не паникует на незнакомом символе
// (в интерактивном чате паника убивала программу), а заменяет его
// пробелом. Если символов не осталось вовсе — возвращает пустой срез,
// вызывающий код обязан это проверить.
func EncodeSafe(s string) []float64 {
        return applyMerges(encodeChars(s, false))
}

func encodeChars(s string, strict bool) []float64 {
        var tokens []float64
        for _, ch := range s {
...
func applyMerges(tokens []float64) []float64 {
